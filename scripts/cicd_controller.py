    ...
    pod_manifest = {
            ...
            'metadata': {
                ...
                'labels':{
                    ...
                    'visible':str(visible_to_user), # used to identify if this pods IP address will be shown to user.
            ...
            'spec': {
                ...
                'imagePullSecrets': [{
                    'name':container_registry_creds_name
                }] 
...
def start_challenge(teamname,challengename):
    ...
    for i in db_pods_data:
        ...
        start_challenge_pod(teamname,k8s_name,image,ram,cpu,storage,visible_to_user,networklist,challengename)
    create_challenge_network_policies(teamname,challengename)
    return 0
    try:
        api = client.NetworkingV1Api()
        policy = create_network_policy_deny_all_task(teamname)
        api_response = api.create_namespaced_network_policy(namespace=teamname, body=policy)
        policy = create_network_policy_allow_task(teamname,challengename,)
...
def get_pods_namespace(teamname,showInvisible):
    ...
    try:
        for pod in pod_list.items:
            ...
            current_pod_info_json=('{"status":"'+state
            +'","ip":"'+pod.status.pod_ip
            +'","visibleIP":'+pod.metadata.labels["visible"]
...
def create_network_policy_deny_all_task(teamname,challengename):
    ...
    policy = client.V1NetworkPolicy(
        ...
        kind="NetworkPolicy",
        metadata=client.V1ObjectMeta(
            name="deny-all-"+sanitized_challengename,
            labels={
                "task":challengename.replace(" ","-")
            }
            ),
        spec=client.V1NetworkPolicySpec(
...
def stop_challenge(teamname,task):
    ...
    try:
        ...
        for policy in policies.items:
            print(f"Deleting NetworkPolicy: {policy.metadata.name}")
...
#        api_version="networking.k8s.io/v1",
#        kind="NetworkPolicy",
#        metadata=client.V1ObjectMeta(name=str(challengename)),
#        spec=client.V1NetworkPolicySpec(
#            pod_selector=client.V1LabelSelector(match_labels={"name": "vpn-container-pod"}),
#            policy_types=["Ingress", "Egress"],
#            ingress=[
